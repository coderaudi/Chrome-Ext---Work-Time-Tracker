// Component loader for extension popup
(async function(){
  ...
  try{
    await loadFragment('comp-header', 'timetracker/components/header.html', '<header class="header-bar"><div class="header-left"><h1>Welcome WTT!</h1><div class="small" id="todayLine"></div></div></header>');
    await loadFragment('comp-clock', 'timetracker/components/clock.html');
    await loadFragment('comp-health', 'timetracker/components/health.html');
    await loadFragment('comp-input', 'timetracker/components/input.html');

    // load health behavior if available
    try{
      const healthScriptSrc = resolve('healthAndWork/health.js');
    ...
    try{
      ...
      try{
        const footerResp = await fetch(resolve('footer/footer.html'));
        if (footerResp.ok){
      ...
      const footerCss = resolve('footer/footer.css');
      ...
      const footerJs = resolve('footer/footer.js');
    ...
    // finally load main timetracker script and call init
    const mainJs = resolve('timetracker/index.js');
    const s = document.createElement('script');
    s.src = mainJs;
    s.onload = function(){ if (window.timetrackerInit) try{ window.timetrackerInit(); }catch(e){console.warn(e)} };
